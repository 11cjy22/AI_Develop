...
def analyze_prediction(data, risk_prob, data_type="股票"):
    ...
    if data_type == "股票":
        ...
        if close < ma20:
            reasons.append(f"收盘价{close:.2f}低于20日均线{ma20:.2f}，中期趋势偏弱")
        else:
            reasons.append(f"收盘价{close:.2f}高于20日均线{ma20:.2f}，中期趋势偏强")
...
def predict_stock_risk(stock_symbol="sh000300"):
    ...
    # 重塑为模型输入格式
    latest_window_scaled = latest_window_scaled.reshape(1, 30, 7)
